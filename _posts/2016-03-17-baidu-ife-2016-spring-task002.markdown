...
因为JS中没有块级作用域，只有全局作用域和函数作用域。使用匿名函数自执行可以直接返回需要的变量（或其他东西）而不用污染全局的作用域。例如下面的例子：
...
```
JS引擎会将以`function`开头的语句解释为函数声明，而函数声明不可以跟圆括号，函数表达式后面可以跟。所以可以使用`()`或其他符号例如`!、~`将之转换为函数表达式立即予以执行